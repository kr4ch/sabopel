...
def gstreamer_init():
    ...
    pipelineStr = "videotestsrc ! video/x-raw, width=1920, height=1080 ! textoverlay text=\"START Screen\" valignment=top halignment=left font-desc=\"Sans, 50\" ! vaapisink fullscreen=true"
    p = Popen(["gstd-client", "pipeline_create", pipelineName, pipelineStr], \
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
    output, err = p.communicate("")

    pipelineName = "pipeCheck"
    pipelineStr = "videotestsrc pattern=ball ! video/x-raw, width=1920, height=1080 !  textoverlay text=\"CHECK Screen\" valignment=top halignment=left font-desc=\"Sans, 50\" ! vaapisink fullscreen=true"
    p = Popen(["gstd-client", "pipeline_create", pipelineName, pipelineStr], \
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
    output, err = p.communicate("")

    pipelineName = "pipeEnd"
    pipelineStr = "videotestsrc pattern=gradient ! video/x-raw, width=1920, height=1080 !  textoverlay text=\"END Screen\" valignment=top halignment=left font-desc=\"Sans, 50\" ! vaapisink fullscreen=true"
    p = Popen(["gstd-client", "pipeline_create", pipelineName, pipelineStr], \
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
    output, err = p.communicate("")
...
def show_camera_start():
    ...
    propertyName = "text"
    propertyValue = "Now starting: " + str(dict_run['team_name'])
