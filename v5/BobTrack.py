...
# defines
timer_switch_enabled = False # camera will only switch automatically when this is set to true. Otherwise when endpoint barrier is passed.
...
def gstreamer_init():
    ...
    pipelineStr = "udpsrc port=5000 caps=\"application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)MP2T-ES\" ! rtpbin ! rtpmp2tdepay ! tsdemux ! h264parse ! avdec_h264 ! videoconvert ! textoverlay text=\"START Screen\" valignment=top halignment=left font-desc=\"Sans, 30\" ! vaapisink fullscreen=true sync=false"
    ...
    pipelineStr = "udpsrc port=5001 caps=\"application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)MP2T-ES\" ! rtpbin ! rtpmp2tdepay ! tsdemux ! h264parse ! avdec_h264 ! videoconvert ! textoverlay text=\"CHECK Screen\" valignment=top halignment=left font-desc=\"Sans, 30\" ! vaapisink fullscreen=true sync=false"
    p = Popen(["gstd-client", "pipeline_create", pipelineName, pipelineStr], \
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
    output, err = p.communicate("")
    ...
    pipelineStr = "udpsrc port=5002 caps=\"application/x-rtp,media=(string)video,clock-rate=(int)90000,encoding-name=(string)MP2T-ES\" ! rtpbin ! rtpmp2tdepay ! tsdemux ! h264parse ! avdec_h264 ! videoconvert ! textoverlay text=\"END Screen\" valignment=top halignment=left font-desc=\"Sans, 30\" ! vaapisink fullscreen=true sync=false"
    p = Popen(["gstd-client", "pipeline_create", pipelineName, pipelineStr], \
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
    output, err = p.communicate("")
...
def show_camera_start():
    ...
    print("start: textoverlay is " + str(elementName))
    propertyName = "text"
...
def show_camera_checkpoint():
    ...
    pipelineName = "pipeCheck"
    elementName = find_element_in_pipe(pipelineName, "textoverlay")
    print("check: textoverlay is " + str(elementName))
    propertyName = "text"
...
def show_camera_endpoint_timer():
    ...
    pipelineName = "pipeEnd"
    elementName = find_element_in_pipe(pipelineName, "textoverlay")
    print("end: textoverlay is " + str(elementName))
    propertyName = "text"
    propertyValue = "Checkpoint Time: " + str("{0:.3f}".format(dict_run['checkpoint_time']))
    p = Popen(["gstd-client", "element_set", pipelineName, elementName, propertyName, propertyValue],
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
...
def show_camera_endpoint_real(_rank):
    ...
    print("end: textoverlay is " + str(elementName))
    propertyName = "text"
    propertyValue = str(dict_run['team_name']) + " Rank: " + str(_rank) + " Total Time: " + str("{0:.3f}".format(dict_run['run_time'])) #+ " Difference to first: " + str("{0:.3f}".format(dict_run['difference_to_first'])) 
    p = Popen(["gstd-client", "element_set", pipelineName, elementName, propertyName, propertyValue],
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
    p = Popen(["gstd-client", "pipeline_play", pipelineName],
        stdin=PIPE, stdout=PIPE, stderr=PIPE)
